...
class SimulatedAnnealing():
    ...
    def evaluate_single_candidate(self, candidates: List[candidateType]):
        # Specifying timeout of 30 minutes
        timeout = 30*60
        # This function both evaluates newly born individuals and store them into the archive
        start = time.time()
        ...
        distance = [self.distance_function(self.Yobs, res) for res in simulated_data]
    ...
    def change_all_parameters(self, old_particle):
        ...
        for key in new_particle:
            ...
            new_particle[key] += 0.1 #* np.random.normal(0, 1)
    ...
    def simple_evaluation(self, particle1, particle2):
        d1 = self.evaluate_single_candidate([particle1])
        d2 = self.evaluate_single_candidate([particle2])
        return particle1 if d1 < d2 else particle2
    ...
    def run_simulation(self) -> None:
        ...
        self.evaluate_candidates(self.population)
