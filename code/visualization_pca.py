...
sa_PCS, sa_EV = load_pickle("../results/sa/sa_pca_R098.pkl")
sa_combined_df_R098 = load_pickle("../results/sa/sa_combined_df_R098.pkl")
frame_ID = sa_combined_df_R098["frame_ID"].to_numpy()
#particle_id = sa_combined_df_R098["particle_ID"].to_numpy()
...
plt.scatter(sa_PCS[:,0], sa_PCS[:, 1],edgecolors=["blue","orange", "green", "red", "purple",][0],
            marker=marker_dict[0],facecolors='none',s=100,label="Simulated Annealing")
plt.xlabel('PC1 ({:.2f}%)'.format(sa_EV[0]*100))
plt.ylabel('PC2 ({:.2f}%)'.format(sa_EV[1]*100))
...
plt.title("Simulated Annealing", fontsize=28, fontweight="bold")
plt.tight_layout()
colors = ['#1f78b4', 'grey', '#fc8d59']
plt.legend(loc="upper right")
plt.savefig(f"../figures/sa_pca_fig_new.png",dpi=400)
