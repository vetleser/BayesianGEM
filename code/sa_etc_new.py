...
class SimulatedAnnealing():
    def __init__(self, 
        ...
        self.current_population: List[candidateType] = []
        self.current_distances: List[float] = []
        self.current_simdata: List[simResultType] = []
        self.candidate_distances: List[float] = []
        self.candidate_simdata: List[simResultType] = []

        self.simdata_tosave: List[simResultType] = []
        self.distances_tosave: List[float] = []
        self.particles_tosave: List[candidateType] = []
    ...
    def evaluate_candidates(self, candidates: List[candidateType]) -> Tuple[List[float], List[simResultType]]: #Taken straight from evo_etc
        ...
        self.candidate_distances = distances
        self.candidate_simdata = simulated_data
    ...
    def energy_function(self, current_dist: float, candidate_dist: float) -> bool:
        ...
        if self.log_ef:
            logging.debug(f"Energy function value is : {acceptance_probability}")
            self.log_ef_list.append(acceptance_probability)
            self.log_ef = False
    ...
    def should_replace(self, current_distance: float, candidate_distance: float) -> bool:
        return self.energy_function(current_distance, candidate_distance)


    

    def replace_population(self, candidates: List[candidateType]):
        logging.info(f"Replacing population with candidates")
        current_population : List[candidateType] = []
        acceptance_counter: int = 0
        for i, (current_distance, candidate_distance) in enumerate(zip(self.current_distances, self.candidate_distances)):
            if self.should_replace(current_distance, candidate_distance):
                if current_distance < -0.5:
                    self.simdata_tosave.append(self.current_simdata[i])
                    self.distances_tosave.append(self.current_distances[i])
                    self.particles_tosave.append(self.current_population[i])
                self.current_distances[i] = self.candidate_distances[i]
                self.current_simdata[i] = self.candidate_simdata[i]
                self.current_population[i] = candidates[i]
                #Keep index in current population as it is
                self.inner_iterations_list[i] = self.min_layers #Add 1 to inner_iterations_list
                logging.debug(f"Replacing particle {i}") #For checking, remove later
                acceptance_counter += 1
            else:
                if candidate_distance < -0.5:
                    self.simdata_tosave.append(self.candidate_simdata[i])
                    self.distances_tosave.append(self.candidate_distances[i])
                    self.particles_tosave.append(candidates[i])
    ...
    def normalize_particle(self, particle: candidateType) -> candidateType:
        min_val = -10
        max_val = 10
        ...
        for parameter, value in particle.items()
        }        
    ...
    def generate_candidates(self, particles: List[candidateType]) -> List[candidateType]: #Kan bruke change_all_parameters her istedenfor å skrive det eksplisitt
        logging.info("Generating candidates")
        candidates: List[candidateType] = []
        if self.normalize:
            for p in particles:
                scaled_candidate = self.normalize_particle(p)
            ...
            for p in particles:
                candidate = {parameter: value for parameter, value in p.items()}
    ...
    def simulate_generation(self):
        ...
        candidates = self.generate_candidates(current_population)
        self.evaluate_candidates(candidates)

        #candidates_idxs = np.flatnonzero(np.array(self.birth_generation) == self.generation)
        self.replace_population(candidates)

        #logging.info(f"Current population is {self.population[-1]}")
        max_generation_epsilon = max(self.current_distances)
        min_generation_epsilon = min(self.current_distances)
    ...
    def run_simulation(self) -> None:
        ...
        if self.generation == 0:
            ...
            self.current_simdata = simulated_data
