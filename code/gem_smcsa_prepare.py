...
candidate_frame = pd.DataFrame(
    index=pd.MultiIndex.from_product(
        [final_temp, move_type, dCpt_step_size, range(n_replicates)],
        names=["final_temp", "move_type", "dCpt_step_size", "simulation"]
    )
).reset_index()
...
if not os.path.exists(outdir):
    os.makedirs(outdir)
candidate_frame['outfile'] = [f'{outdir}/smcsa_gem_may31_{dCpt_step_size}_{simulation}.pkl' for
 dCpt_step_size, simulation in zip(candidate_frame['dCpt_step_size'],candidate_frame['simulation'])]
